  ...
  return (
    <div className="site-shell">
      ...
      {active === "overview" && <main>
        ...
        <section className="stats-band"><Stat value="4,475" label="approved products" detail="underlying source statement" /><Stat value="60.2%" label="mapped to MoA" detail="2,694 rows mapped; 2,209 use current IRAC" /><Stat value="45" label="MoA groups" detail="IRAC · FRAC · HRAC" /><Stat value="01" label="decision rule" detail="never repeat a group" /></section>
      ...
      {selected && <div className="modal-backdrop" onClick={() => setSelected(null)}><div className="product-modal" onClick={(e) => e.stopPropagation()}><button className="modal-close" onClick={() => setSelected(null)} aria-label="Close"><X size={18} /></button><span className="section-kicker">PRODUCT DETAIL / SAMPLE ROW</span><h2>{selected.trade}</h2><p className="modal-subtitle">{selected.ingredient} · {selected.category}</p><div className="modal-grid"><div><span>Registration</span><b>{selected.reg}</b></div><div><span>Mode of action</span><b>{selected.moa}</b><small className="mapping-detail">{selected.mappingStatus} · {selected.mappingConfidence} confidence</small></div><div><span>Crop</span><b>{selected.crop}</b></div><div><span>Target</span><b>{selected.pest}</b></div><div><span>Dosage</span><b>{selected.dosage}</b></div><div><span>Registration holder</span><b>{selected.holder}</b></div></div><div className="modal-note"><ShieldCheck size={16} /><span>{selected.mappingSources.length ? `Mapping source: ${selected.mappingSources.join(" · ")}` : "No authoritative MoA mapping is attached to this row yet."} Check the current product label before field use.</span></div></div></div>}
    </div>
  );
}
