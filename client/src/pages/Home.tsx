...
import { seedOrLoadCatalog } from "@/lib/catalogDb";
...
export default function Home() {
  ...
  useEffect(() => {
    seedOrLoadCatalog(products).then((saved) => { setCatalog(saved); setCatalogReady(true); }).catch(() => setCatalogReady(true));
  }, []);
  const copy = bn ? {
  ...
  const pestOptions = useMemo(() => ["All pests", ...Array.from(new Set(catalog.flatMap((p) => p.pest.split(" · ")).filter(Boolean)).values()).sort()], []);
  const categoryOptions = useMemo(() => ["All categories", ...Array.from(new Set(catalog.map((p) => p.category).filter(Boolean)).values()).sort()], []);
  const filteredProducts = useMemo(() => catalog.filter((p) => {
    const q = query.toLowerCase();
    return (!q || [p.trade, p.ingredient, p.reg, p.crop, p.pest, p.moa].join(" ").toLowerCase().includes(q)) &&
      (scheme === "All systems" || (scheme === "Unmapped" ? p.mappingStatus === "unmapped" : p.moaCodes.some((code) => code.startsWith(scheme)))) &&
      (cropFilter === "All crops" || p.crop.split(" · ").includes(cropFilter)) &&
      (pestFilter === "All pests" || p.pest.split(" · ").includes(pestFilter)) &&
      (categoryFilter === "All categories" || p.category === categoryFilter);
  }), [catalog, query, scheme, cropFilter, pestFilter, categoryFilter]);
    ...
    <div className="site-shell">
      <header className="topbar">
        <div className="brand-lockup" onClick={() => navigate("overview")} role="button" tabIndex={0}>
          <div className="brand-mark"><img className="brand-logo-image" src="/manus-storage/agrichem-pro-logo_5c407210.png" alt="AgriChem Pro logo" /></div>
