...
export default function Home() {
  ...
  const cropOptions = useMemo(() => ["All crops", ...Array.from(new Set(products.flatMap((p) => p.crop.split(" · "))).values()).sort()], []);
  const pestOptions = useMemo(() => ["All pests", ...Array.from(new Set(products.flatMap((p) => p.pest.split(" · "))).values()).sort()], []);
  const categoryOptions = useMemo(() => ["All categories", ...Array.from(new Set(products.map((p) => p.category)).values()).sort()], []);
